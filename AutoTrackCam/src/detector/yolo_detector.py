...
class YOLODetector:
    ...
    def __init__(
        ...
        engine_output_names = [
            info.name for info in self.trt_engine.get_output_details()
        ]
        for key, name in self.output_names.items():
            if name not in engine_output_names:
                print(
                    f"Warning: Expected output tensor '{name}' for '{key}' not found in engine. "
    ...
    def detect(
        ...
        try:
            num_dets = outputs_dict[self.output_names["num_dets"]][
                0
            ].item()  # Get the scalar value
            # Squeeze batch dimension if present
            bboxes_letterboxed = (
                outputs_dict[self.output_names["bboxes"]][0, :num_dets, :].cpu().numpy()
            )  # (N, 4)
            scores = (
                outputs_dict[self.output_names["scores"]][0, :num_dets].cpu().numpy()
            )  # (N,)
            class_ids = (
                outputs_dict[self.output_names["labels"]][0, :num_dets]
                .cpu()
                .numpy()
                .astype(np.int32)
            )  # (N,)
            ...
            print(
                f"Error: Output tensor name '{e}' not found in TRT engine outputs. Check self.output_names."
            )
...
if __name__ == "__main__":
    ...
    if not config.YOLO_ENGINE_PATH.exists():
        ...
        print(
            "Please run 'scripts/download_models.sh' and 'scripts/export_trt_engines.sh' first."
        )
    else:
        try:
            ...
            if bboxes.shape[0] > 0:
                ...
                for i in range(min(5, bboxes.shape[0])):  # Print first 5 detections
                    print(
                        f"  Box: {bboxes[i]}, Score: {scores[i]:.2f}, Class ID: {class_ids[i]} ({config.CLASSES[class_ids[i]]})"
                    )
            else:
