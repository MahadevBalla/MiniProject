...
class TRTEngine(torch.nn.Module):
    ...
    def __init__(
        self, engine_path: Union[str, Path], device: Optional[torch.device] = None
    ):
        super().__init__()
        self.engine_path = (
        ...
        if self.device.type == "cpu":
            print(
                "Warning: TRTEngine is intended for CUDA devices, but CPU device was selected."
            )
            self.engine = None
            self.context = None
    ...
    def _init_engine(self):
        if not self.engine_path.exists():
            raise FileNotFoundError(
                f"TensorRT engine file not found: {self.engine_path}"
            )
    ...
    def _init_bindings_info(self):
        ...
        for i in range(self.engine.num_io_tensors):  # type: ignore
            ...
            tensor_details = TensorInfo(
                name=name, dtype=dtype, shape=shape, is_dynamic=is_dynamic
            )
    ...
    def _get_example_input_shape(self, tensor_info: TensorInfo) -> Tuple[int, ...]:
        ...
        if len(current_profile_shape) == 4:  # Assuming NCHW
            final_shape.append(
                1 if current_profile_shape[0] == -1 else current_profile_shape[0]
            )  # Batch
            final_shape.append(
                current_profile_shape[1] if current_profile_shape[1] != -1 else 3
            )  # Channels
            final_shape.append(
                example_config_shape_hw[0]
                if current_profile_shape[2] == -1
                else current_profile_shape[2]
            )  # Height
            final_shape.append(
                example_config_shape_hw[1]
                if current_profile_shape[3] == -1
                else current_profile_shape[3]
            )  # Width
    ...
    def _warm_up(self, iterations=5):
        ...
        for info in self.input_info_list:
            if info.is_dynamic:
                ...
                print(
                    f"  Warm-up: Dynamic input '{info.name}' using example shape {concrete_shape}."
                )
                dummy_inputs_dict[info.name] = torch.randn(
                    concrete_shape, dtype=info.dtype, device=self.device
                )
            else:  # Static input
                dummy_inputs_dict[info.name] = torch.randn(
        ...
        if any_dynamic:
            print(
                f"Warning: Engine '{self.engine_path.name}' has dynamic inputs. Warm-up with example shapes."
            )

        print(
            f"Warming up engine '{self.engine_path.name}' for {iterations} iterations..."
        )
        ...
        end_time = time.time()
        print(
            f"Warm-up for '{self.engine_path.name}' finished in {end_time - start_time:.3f} seconds."
        )
    ...
    def infer(self, inputs: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        for info in self.input_info_list:
            ...
            if input_tensor.dtype != info.dtype:
                print(
                    f"Warning: Input tensor '{info.name}' dtype mismatch. "
        ...
        for info in self.output_info_list:
            ...
            outputs_dict[info.name] = torch.empty(
                output_shape, dtype=info.dtype, device=self.device
            )
        ...
        if not self.context.execute_async_v3(stream_handle=stream.cuda_stream):  # type: ignore
            raise RuntimeError(
                f"TensorRT execute_async_v3() failed for engine {self.engine_path.name}"
            )
    ...
    def __call__(self, inputs: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        if not isinstance(inputs, dict):
            raise TypeError(
                f"Input to {self.engine_path.name} TRTEngine must be a dictionary mapping "
