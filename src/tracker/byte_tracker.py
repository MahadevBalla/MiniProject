...
class BYTETracker(object):
    ...
    def update(self, output_results, img_info, img_size):
        ...
        if output_results.shape[1] == 5:
            ...
            scores = output_results[:, 4]
            bboxes = output_results[:, :4]
            class_ids = output_results[:, 5].astype(np.float32)
        else:
            ...
            class_ids = (
                ...
                else output_results[:, 6].astype(np.float32)
            )
        ...
        if len(dets) > 0:
            ...
            if class_ids is not None:
                detections = [
                    STrack(STrack.tlbr_to_tlwh(tlbr), s, int(c))
                    for (tlbr, s, c) in zip(dets, scores_keep, class_ids[remain_inds])
                ]
            else:
                detections = [
                    STrack(STrack.tlbr_to_tlwh(tlbr), s, None)
        ...
        if len(dets_second) > 0:
            ...
            if class_ids is not None:
                detections_second = [
                    STrack(STrack.tlbr_to_tlwh(tlbr), s, int(c))
                    for (tlbr, s, c) in zip(
                        dets_second, scores_second, class_ids[inds_second]
                    )
                ]
            else:
                detections_second = [
