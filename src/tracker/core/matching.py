...
def ious(atlbrs, btlbrs):
    ...
    a = np.ascontiguousarray(np.asarray(atlbrs, dtype=np.float64)).reshape(-1, 4)
    b = np.ascontiguousarray(np.asarray(btlbrs, dtype=np.float64)).reshape(-1, 4)
...
def iou_distance(atracks, btracks):
    ...
    if len(atlbrs) > 0:
        ...
        atlbrs = np.empty((0, 4), dtype=np.float64)

    if len(btlbrs) > 0:
        btlbrs = np.asarray(btlbrs, dtype=np.float64).reshape(-1, 4)
    else:
        btlbrs = np.empty((0, 4), dtype=np.float64)
