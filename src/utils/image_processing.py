...
def letterbox(
    ...
    dw, dh = (
        new_shape[1] - new_unpad_shape[1],
        new_shape[0] - new_unpad_shape[0],
    ...
    if shape[::-1] != new_unpad_shape:  # resize
        im = cv2.resize(
            im, (new_unpad_shape[1], new_unpad_shape[0]), interpolation=cv2.INTER_LINEAR
        )  # W, H for cv2.resize
    ...
    return (
        ...
        (r, r),
        (dw, dh),
    )  # return same ratio for h & w for simplicity in unscaling


def preprocess_yolo_input(
    image_bgr: np.ndarray, target_shape: Tuple[int, int]
) -> np.ndarray:
    """
    ...
    """
    img_letterboxed, ratios, (pad_w, pad_h) = letterbox(
        image_bgr, new_shape=target_shape, auto=False, scaleup=False
    )  # Use auto=False for standard letterboxing
...
def preprocess_reid_input(
    image_crop_bgr: np.ndarray, target_shape: Tuple[int, int] = (128, 64)
) -> np.ndarray:
    """
    ...
    resized_img = cv2.resize(
        image_crop_bgr, (target_shape[1], target_shape[0])
    )  # W, H for cv2.resize
